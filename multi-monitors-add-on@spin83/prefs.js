...
const Convenience = './convenience.js';
...
function langCallback(sourceObject, arg1) {
    if (this === sourceObject) {
        log('`sourceObject` is correctly bound to `this`');
    }
}

const MultiMonitorsPrefsWidget = GObject.registerClass(
    class MultiMonitorsPrefsWidget extends Gtk.Grid {
        _init() {
            ...
            this._settings.connect(`changed::${TRANSFER_INDICATORS_ID}`, langCallback.bind(this, this._updateIndicators));
            this._updateIndicators();

            let addTButton = new Gtk.Button({icon_name: 'list-add'});
            addTButton.connect('clicked', langCallback.bind(this, this._addIndicator));
            toolbar.append(addTButton);

            let removeTButton = new Gtk.Button({icon_name: 'list-remove'});
            removeTButton.connect('clicked', langCallback.bind(this, this._removeIndicator));
...
export default class MultiMonitorsAddonPreferences extends ExtensionPreferences {
    fillPreferencesWindow() {
        Convenience.initTranslations();

        let preferences = new MultiMonitorsPrefsWidget();
    }
}
